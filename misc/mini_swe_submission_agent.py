...
BASH_TOOL = {
    ...
    "function": {
        "name": "bash",
        "description": "Execute one bash command in the repository.",
        "parameters": {
            "type": "object",
            "properties": {
                "command": {
...
def _chat_tools_payload() -> List[Dict[str, Any]]:
    return [BASH_TOOL]
...
def extract_tool_command(raw_response: Dict[str, Any]) -> Optional[str]:
    try:
...
def build_assistant_message(response_text: str, raw_response: Dict[str, Any]) -> Dict[str, Any]:
    try:
        message = dict(raw_response["choices"][0]["message"])
    except Exception:
        return {"role": "assistant", "content": response_text}
    message.setdefault("role", "assistant")
    message["content"] = message.get("content") or response_text
    return message


def build_observation_message(observation: str, raw_response: Dict[str, Any]) -> Dict[str, str]:
    try:
        calls = raw_response["choices"][0]["message"].get("tool_calls") or []
        call_id = calls[0].get("id")
    except Exception:
        call_id = None
    if call_id:
        return {"role": "tool", "tool_call_id": str(call_id), "content": observation}
...
def extract_marker_submission(observation: str) -> Optional[str]:
    lines = observation.lstrip().splitlines()
    for idx, line in enumerate(lines):
        if line.strip() == SUBMIT_MARKER:
            return "\n".join(lines[idx + 1 :]).strip() or "submitted"
    return None
...
def _has_meaningful_patch(patch: str) -> bool:
    if len(patch.strip()) < MIN_MEANINGFUL_PATCH_CHARS:
        return False
    changed_lines = [
        line
...
SYSTEM_PROMPT = """You are mini-swe-agent, a simple coding agent running inside a repository.

Your only action interface is bash. Use bash commands to inspect files, edit
files, run tests, and submit. Each command runs independently, so persist any
important state in files or in the next command.

Prefer the bash tool call when available. If tool calls are unavailable, use this
...
When you are finished, run a bash command whose first output line is:
COMPLETE_TASK_AND_SUBMIT_FINAL_OUTPUT
Then print a short summary on following lines. If tool calls are unavailable,
you may also use:
...
def solve(
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            try:
                response_text, cost, raw_response = chat_completion(
                    messages=messages,
                    model=model_name,
                    api_base=api_base,
                    api_key=api_key,
            ...
            command = extract_tool_command(raw_response) or extract_command(response_text)

            if command is None:
                messages.append(build_assistant_message(response_text, raw_response))
            ...
            messages.append(build_assistant_message(response_text, raw_response))
            messages.append(build_observation_message(observation, raw_response))

            marker_submission = extract_marker_submission(result.stdout)
        ...
        patch = get_patch(repo)
        if patch.strip() and not success and _has_meaningful_patch(patch):
            logs.append("\nAUTO_SUCCESS:\nMeaningful patch exists even though the model did not emit a final marker.")
            success = True
        ...
        return AgentResult(
            ...
            cost=total_cost,
            success=success and _has_meaningful_patch(patch),
        ).to_dict()
